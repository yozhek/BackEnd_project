...
describe("products API", () => {
  ...
  it("creates product without discount (discountPercent defaults to 0)", async () => {
    const res = await request(app)
      .post("/products")
      .send({title:"PC", price:1000, category:"electronics"})
      .set("Content-Type","application/json")
    expect(res.status).toBe(201)
  ...
  it("rejects invalid input (discountPercent 0 when provided)", async () => {
    const res = await request(app)
      .post("/products")
      .send({title:"X", price:100, discountPercent:0, category:"electronics"})
      .set("Content-Type","application/json")
  ...
  it("lists products with pagination", async () => {
    ...
    await db.collection("products").insertMany([
      {title:"A", price:10, discountPercent:0, discountedPrice:10, category:"c", createdAt:new Date(), updatedAt:new Date()},
      {title:"B", price:20, discountPercent:0, discountedPrice:20, category:"c", createdAt:new Date(), updatedAt:new Date()},
      {title:"C", price:30, discountPercent:0, discountedPrice:30, category:"c", createdAt:new Date(), updatedAt:new Date()}
    ])
