...
describe("orders API", () => {
  it("creates order", async () => {
    const prod1 = await createProduct({title:"A", price:100, discountPercent:10, category:"c", description:"d", imageBase64:"data:image/png;base64,xx"})
    const prod2 = await createProduct({title:"B", price:50, category:"c", description:"d"})
  ...
  it("rejects invalid order (empty items)", async () => {
    const res = await request(app).post("/orders").send({items: [], status:"pending"})
  ...
  it("lists orders with pagination", async () => {
    const prod = await createProduct({title:"A", price:10, category:"c", description:"d"})
    await request(app).post("/orders").send({items:[{productId: prod, quantity:1}], status:"pending"})
    await request(app).post("/orders").send({items:[{productId: prod, quantity:1}], status:"pending"})
  ...
  it("gets order by id", async () => {
    const prod = await createProduct({title:"A", price:10, category:"c", description:"d"})
    const created = await request(app).post("/orders").send({items:[{productId: prod, quantity:1}], status:"pending"})
